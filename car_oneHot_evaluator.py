# -*- coding: utf-8 -*-
"""
Created on Sun Dec 31 14:56:15 2017
...
oneh_enc.fit(data[:,0:6])
X_hot_train = oneh_enc.transform(X_train).toarray()
X_hot_test = oneh_enc.transform(X_test).toarray()

print('\nOne Hot Encoder n_values: ',oneh_enc.n_values_, '\n')
...
predicted_clf = clf.predict(X_hot_test)
...
# no visualization for one-hot encoded decision tree
...
nby = GaussianNB()
nby.fit(X_hot_train, y_train)
print('\n\nNaive_bayes_classifier:')
print(nby)
# make predictions
expected_nby = y_test
predicted_nby = nby.predict(X_hot_test)
# summarize the fit of the model
...
knb = KNeighborsClassifier(algorithm='auto', leaf_size=30, metric='minkowski',
                           ...
                           weights='uniform')
knb.fit(X_hot_train, y_train)
print('\n\nK_nearest_neighbors_classifier:')
print(knb)
# make predictions
expected_knb = y_test
predicted_knb = knb.predict(X_hot_test)
# summarize the fit of the model
...
svmc.fit(X_hot_train, y_train)
print('\n\nSVM_classifier:')
print(svmc)
# make predictions
expected_svmc = y_test
predicted_svmc = svmc.predict(X_hot_test)
# summarize the fit of the model
...
mlp.fit(X_hot_train, y_train)
print('\n\nMLP_classifier:')
print(mlp)
# make predictions
expected_mlp = y_test
predicted_mlp = mlp.predict(X_hot_test)
# summarize the fit of the model
